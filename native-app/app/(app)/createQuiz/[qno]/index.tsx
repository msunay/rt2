import { addQuestionWithAnswers, resetQuizStore } from '@/src/features/quizCreationSlice';
import { useAppDispatch, useAppSelector } from '@/src/hooks/reduxHooks';
import { useAddFullQuizMutation } from '@/src/api/backendApi';
...
export default function QuizContent() {
  ...
  const [currentRef, setCurrentRef] = useState<RefObject<TextInput | null>>(ref_input1);

  // Focus next input
  const focusNextInput = (currentRef: RefObject<TextInput | null>) => {
    switch (currentRef) {
      case ref_input1:
  ...
  const focusPreviousInput = (currentRef: RefObject<TextInput | null>) => {
    switch (currentRef) {
