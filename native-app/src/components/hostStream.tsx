...
export default function HostStream({ quizId }: { quizId: string }) {
  ...
  const { quizStarted, questionHidden, trigger } = useAppSelector(
    (state) => state.quizSlice
  );
  const { mediaStream, isConnecting } = useAppSelector(
  ...
  const { getLocalStream, connect, endStream } =
    useMediasoup(streamSocketManager);
