...
export class Logger {
     ...
     * @param logConfig Optional logger configuration
     */
    constructor(namespace?: string, logConfig: Partial<LoggerConfig> = {}) {
        this.logConfig = {
            ...defaultConfig,
            ...logConfig,
            namespace,
    ...
    private log(level: LogLevel, message: string, ...data: any[]): void {
        ...
        if (logLevelPriority[level] < logLevelPriority[this.logConfig.minLevel]) {
            return;
        }

        // Format message with namespace if it exists
        const formattedMessage = this.logConfig.namespace ? `[${this.logConfig.namespace}] ${message}` : message;
     ...
     * @param data Additional data to log
     */
    public logError = (message: string, error?: Error | unknown, ...data: any[]): void => {
    ...
    public child(childNamespace: string): Logger {
        const parentNamespace = this.logConfig.namespace;
        const combinedNamespace = parentNamespace ? `${parentNamespace}:${childNamespace}` : childNamespace;

        return new Logger(combinedNamespace, this.logConfig);
    }
}
