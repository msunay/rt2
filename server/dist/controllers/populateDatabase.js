...
async function populateDatabase() {
    ...
    await index_1.default.User.create(mockHost);
    await index_1.default.User.create(mockPlayer);
    // Create the quiz
    const quiz = await index_1.default.Quiz.create({
        quizName: "Mock Quiz",
        quizOwner: mockHost.id,
        category: "General Knowledge",
        dateTime: (0, moment_1.default)().add(5, "days").toDate(),
    });
    // Create questions and answers
    for (let i = 1; i <= 10; i++) {
        const question = await index_1.default.Question.create({
    ...
    await index_1.default.Participation.create({
