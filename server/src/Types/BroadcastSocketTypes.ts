...
export interface BroadcastListenEvents {
  create_room: (
    callback: (response: SocketResponse<{ rtpCapabilities: RtpCapabilities }>) => void
  ) => void;
  ...
  createWebRtcTransport: (
    ...
    callback: (response: SocketResponse<{ transportOptions: TransportOptions }>) => void
  ) => void;

  transport_connect: (
    opts: { transportId: string; dtlsParameters: DtlsParameters }
  ) => void;
  ...
  consume: (
    ...
    callback: (response: SocketResponse<{ consumerOptions: ConsumerOptions }>) => void
  ) => void;

  consumer_resume: ({ consumerId }: { consumerId: string; }) => void;
}
